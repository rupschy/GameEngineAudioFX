...
const double CUTOFF = 1000.0; // cutoff frequency of the filter in Hz
...
int main() {
  ...
  for (int i = 0; i < samples.size() - HOP_SIZE; i += HOP_SIZE) {
    ...
    // Filter the frame using a high-pass filter
    std::vector<int16_t> filtered_frame(HOP_SIZE);
    // Calculate the filter coefficients
    double w0 = 2 * M_PI * CUTOFF / 44100.0;
    double alpha = std::sin(w0) / (2 * Q);
    double a0 = 1 + alpha;
    double b0 = (1 + std::cos(w0)) / 2;
    double b1 = -(1 + std::cos(w0));
    double b2 = (1 + std::cos(w0)) / 2;
    ...
    if (i + HOP_SIZE >= samples.size()) {
      filtered.resize(samples.size());
      std::copy(filtered.begin(), filtered.end(), samples.begin());
    }
    }
