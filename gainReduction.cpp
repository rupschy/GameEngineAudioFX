...
const double THRESHOLD = -10.0; // threshold of the gain reduction in dB
const double RATIO = 2.0; // ratio of the gain reduction
const double KNEE = 0.0; // knee of the gain reduction in dB
...
int main() {
  ...
  for (int i = 0; i < samples.size() - HOP_SIZE; i += HOP_SIZE) {
    ...
    // Process the frame using a gain reduction effect
    ...
    for (int j = 0; j < HOP_SIZE; j++) {
      ...
      // Apply the gain reduction effect
      ...
      if (KNEE > 0.0 && input_gain > knee) {
        ...
        double soft_knee = (input_gain - knee) / slope;
